...
### 2) change xaxis labels to clock time of 0:00 - 24:00
  ...
  if (length(dimfd) == 2) {
    for (jharm in 1:length(harm)) {
      ...
      plot(x, meanmat, type = "l", ylim = c(min(pcmat), 
                                            max(pcmat)), ylab = NA, 
           main = paste("FPC",iharm, " ", "(", percentvar, "% variability)", sep=""), 
           xaxt="n", xlab=NA , ...
      )
      ...
      if (pointplot) {
        points(x, pcmat[, 1], pch = "+", col="orange2")
        points(x, pcmat[, 2], pch = "-", col="blue3")
      }
      else {
        lines(x, pcmat[, 1], lty = 2, col="orange2")
        lines(x, pcmat[, 2], lty = 3, col="blue3")
      }
    }
  }
